...
const outAssPath = process.argv[3] ?? "work/harbor/captions/harbor-dynamic-word-by-word.ass";
const outTimingPath = process.argv[4] ?? "work/harbor/captions/harbor-dynamic-corrected-word-timings.json";
...
type WordTiming = { text: string; norm: string; start: number; end: number };
type LyricLine = { text: string; section: string };
type ExactWord = { text: string; norm: string; lineIndex: number; wordInLine: number; section: string; start?: number; end?: number; revealStart?: number; matched?: string };
type Tier = { name: string; fontSize: number; y: number; maxWords: number; marginV: number; outline: number; shadow: number; lift: number };

const ivory = "&H00EEF7FF";
const gold = "&H003FCBFF";
const hotGold = "&H001E96FF";
const ember = "&H000E78FF";
const outline = "&H00100A05";
...
function parseLyricLines(script: string): LyricLine[] {
  const lines: LyricLine[] = [];
  let section = "Opening Scene";
  for (const raw of script
    ...
    if (!line) continue;
    if (line.startsWith("[")) {
      if (!line.toLowerCase().includes("immediately")) section = line.replace(/^\[/, "").replace(/\]$/, "");
      continue;
    }
...
function tierForSection(section: string): Tier {
  if (/Massive Climax|Climax Extension/i.test(section)) return { name: "Climax", fontSize: 190, y: 1000, maxWords: 4, marginV: 44, outline: 10, shadow: 6, lift: 24 };
  if (/False Summit|Pre-Climax|Final Refrain/i.test(section)) return { name: "Chorus", fontSize: 173, y: 1000, maxWords: 4, marginV: 48, outline: 9.5, shadow: 5.5, lift: 18 };
  if (/Long Crescendo|Ascending Passage|Final Shore/i.test(section)) return { name: "Lift", fontSize: 163, y: 1000, maxWords: 4, marginV: 52, outline: 9, shadow: 5, lift: 14 };
  if (/Deepest Low|Quiet Refrain/i.test(section)) return { name: "Intimate", fontSize: 156, y: 1000, maxWords: 4, marginV: 54, outline: 8.5, shadow: 5, lift: 10 };
  return { name: "Verse", fontSize: 156, y: 1000, maxWords: 4, marginV: 56, outline: 8.5, shadow: 5, lift: 12 };
}
...
function parseWhisperWords(path: string): WordTiming[] {
  ...
  for (const segment of data.transcription ?? []) {
    ...
    const normalizedSegmentText = segmentText.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    const segmentStart = Number(segment.offsets?.from ?? timeFromWhisper(segment.timestamps?.from ?? "0:00:00,000"));
    const segmentEnd = Number(segment.offsets?.to ?? timeFromWhisper(segment.timestamps?.to ?? "0:00:00,000"));
    const segmentDurationSeconds = Math.abs(segmentEnd - segmentStart) > 1000
    ...
    if (normalizedSegmentText) previousSegmentText = normalizedSegmentText;
    ...
    for (const token of segment.tokens ?? []) {
      ...
      const startS = hasOffsets ? Number(token.offsets.from) / 1000 : timeFromWhisper(token.timestamps?.from ?? "0:00:00,000");
      const endS = hasOffsets ? Number(token.offsets.to) / 1000 : timeFromWhisper(token.timestamps?.to ?? "0:00:00,000");
      ...
      for (let i = 0; i < pieces.length; i += 1) {
        ...
        words.push({ text: piece, norm, start: startS + step * i, end: startS + step * (i + 1) });
      }
    }
  }
...
function align(exact: ExactWord[], heard: WordTiming[]) {
  ...
  for (let i = 1; i <= n; i += 1) { dp[i][0] = dp[i - 1][0] + gapExact; move[i][0] = 1; }
  for (let j = 1; j <= m; j += 1) { dp[0][j] = dp[0][j - 1] + gapHeard; move[0][j] = 2; }
  for (let i = 1; i <= n; i += 1) {
    for (let j = 1; j <= m; j += 1) {
  ...
  while (i > 0 || j > 0) {
    ...
    if (mv === 0) {
      const e = exact[i - 1];
      const h = heard[j - 1];
      e.start = h.start; e.end = h.end; e.matched = h.text;
      i -= 1; j -= 1;
    } else if (mv === 1) i -= 1;
    else j -= 1;
  }
...
function computeRevealStarts(words: ExactWord[]) {
  for (let i = 0; i < words.length; i += 1) {
    const word = words[i];
    let revealStart = word.start!;

    // DTW makes Whisper begin times usable. Still guard against pathological
    // non-monotonic or zero-duration tokens by clamping just before the word end.
    if (revealStart >= word.end!) revealStart = Math.max(0, word.end! - 0.12);

    word.revealStart = Math.max(0, revealStart);
  }
}

function chunkLine(lineWords: ExactWord[]) {
  const tier = tierForSection(lineWords[0]?.section ?? "");
  const chunks: ExactWord[][] = [];
  let current: ExactWord[] = [];
  let chars = 0;
  const maxChars = tier.name === "Climax" ? 28 : tier.name === "Chorus" ? 32 : 34;
  for (const word of lineWords) {
    const extra = (current.length ? 1 : 0) + word.text.length;
    if (current.length && (current.length >= tier.maxWords || chars + extra > maxChars)) {
      chunks.push(current);
      current = [];
      chars = 0;
    }
...
const emphasis = new Set(["harbor", "lighthouse", "flame", "fire", "storm", "stormwall", "thunder", "sea", "waves", "wave", "dawn", "destiny", "rise", "raging", "roar", "spark", "sail", "gold", "victory", "more"]);

function wordBlock(word: ExactWord, eventStart: number, tier: Tier) {
  const rel = Math.max(0, Math.round(((word.revealStart ?? word.start!) - eventStart) * 1000));
  const appear = rel + 120;
  const settle = rel + 420;
  const isEmphasis = emphasis.has(word.norm);
  const finalColor = isEmphasis ? gold : ivory;
  const initialColor = isEmphasis ? hotGold : gold;
  const peakScale = tier.name === "Climax" ? 116 : tier.name === "Chorus" ? 113 : 110;
  const startScale = tier.name === "Climax" ? 82 : 86;
  const blurPeak = isEmphasis ? 2.6 : 1.8;
  const finalBlur = 0.4;
  const extra = isEmphasis ? `\\bord${(tier.outline + 1).toFixed(1)}` : `\\bord${tier.outline}`;
  return `{\\alpha&HFF&\\c${initialColor}&\\fscx${startScale}\\fscy${startScale}\\blur${blurPeak}${extra}\\t(${rel},${appear},\\alpha&H00&\\fscx${peakScale}\\fscy${peakScale})\\t(${appear},${settle},\\fscx100\\fscy100\\blur${finalBlur}\\c${finalColor}&\\bord${tier.outline})}` + escapeAss(word.text);
}

function splitForTwoLines(chunk: ExactWord[]) {
  const rows: ExactWord[][] = [];
  let current: ExactWord[] = [];
  let chars = 0;
  for (const word of chunk) {
    const extra = (current.length ? 1 : 0) + word.text.length;
    if (current.length && (current.length >= 4 || chars + extra > 36)) {
      rows.push(current);
      current = [];
      chars = 0;
    }
...
function chunkText(chunk: ExactWord[], eventStart: number, tier: Tier) {
  return splitForTwoLines(chunk)
    .map((line) => line.map((word) => wordBlock(word, eventStart, tier)).join(" "))
    .join("\\N");
}

function impactLabelForLine(line: string) {
  const norm = line.toLowerCase();
  if (norm.startsWith("sail, sail")) return "SAIL, SAIL";
  if (norm.startsWith("rise, rise")) return "RISE, RISE";
  if (norm.startsWith("no ocean can own me")) return "NO OCEAN CAN OWN ME";
  if (norm.startsWith("i am the sail")) return "I AM THE SAIL";
  if (norm.startsWith("i am the storm")) return "THE STORM LEARNED MY NAME";
  if (norm.startsWith("so i will carve")) return "CARVE MY DESTINY";
  if (norm.startsWith("through the thunder")) return "THROUGH THE THUNDER";
  if (norm.startsWith("i rise like")) return "RISE LIKE THE SUN";
  if (norm.startsWith("then the clouds were torn")) return "CLOUDS TORN ASUNDER";
  if (norm.startsWith("and because of")) return "SOMETHING MORE";
  return null;
}

const lines = parseLyricLines(lyricScript);
const exactWords: ExactWord[] = [];
lines.forEach((line, lineIndex) => {
  line.text.split(/\s+/).forEach((text, wordInLine) => {
    const norm = normalizeWord(text);
    if (!norm) return;
    exactWords.push({ text, norm, lineIndex, wordInLine, section: line.section });
  });
});
...
computeRevealStarts(exactWords);

const chunks: ExactWord[][] = [];
for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
  const lineWords = exactWords.filter((word) => word.lineIndex === lineIndex);
  if (lineWords.length) chunks.push(lineWords);
}

const header = `[Script Info]
Title: Harbor dynamic word-by-word captions
ScriptType: v4.00+
WrapStyle: 0
...
Style: Lyric, Old Master, 92, ${ivory}, ${gold}, ${outline}, &H9A000000, 0, 0, 0, 0, 100, 100, 0.6, 0, 1, 5.5, 3.5, 2, 120, 120, 96, 1
Style: Impact, Old Master, 122, &H65FFFFFF, ${gold}, &H8A05020A, &H00000000, 0, 0, 0, 0, 105, 105, 2, 0, 1, 7, 1, 5, 100, 100, 60, 1
...
for (let i = 0; i < chunks.length; i += 1) {
  ...
  const tier = tierForSection(chunk[0].section);
  const first = chunk[0];
  const last = chunk[chunk.length - 1];
  const start = Math.max(0, (first.revealStart ?? first.start!) - 0.22);
  let end = Math.min(last.end! + 1.12, nextChunk ? nextChunk[0].start! - 0.07 : last.end! + 1.5);
  if (end <= start + 0.35) end = start + 0.75;
  const fromY = tier.y + tier.lift;
  const toY = tier.y;
  const lineCount = splitForTwoLines(chunk).length;
  const pos = `{\\an2\\move(960,${fromY},960,${toY},0,${Math.max(450, Math.round((end - start) * 1000))})\\fs${tier.fontSize}\\bord${tier.outline}\\shad${tier.shadow}\\blur0.35\\fsp0.6}`;
  const body = pos + chunkText(chunk, start, tier);
  const marginV = lineCount > 1 ? Math.max(70, tier.marginV - 8) : tier.marginV;
  events.push(`Dialogue: 1,${assTime(start)},${assTime(end)},Lyric,,0,0,${marginV},,${body}`);
}
...
writeFileSync(outTimingPath, JSON.stringify({
  ...
  words: exactWords.map((word) => ({
    ...
    revealStart: Number((word.revealStart ?? word.start!).toFixed(3)),
