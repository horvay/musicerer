...
- LTX video prompts (`videoPrompt`, node `3305.inputs.text`) remain one flowing present-tense cinematic paragraph focused on what changes over time: camera movement, character action, weather/cloth/hair/environment motion, light changes, and emotional beat.
...
- Use `music-video-clip-maker` for generating Flux stills or LTX video attempts through the approved workflows.
...
1. Transcribe the song with local Whisper.
...
3. Director agent creates/updates a visual plan from lyrics and song structure, including Flux natural-language `imagePrompt` values. The director must validate scene timings against any user-specified clip duration range before generation begins.
...
9. Move to the next scene's still image only after the critic has approved the current scene image or the 30-attempt cap has selected the best available image. Do not generate any LTX videos until **all scenes** have approved/selected Flux still images.
10. Video pass second: generate LTX video attempts scene by scene in order from the approved Flux images.
...
- Video workflow: `music_vid_comfyui_video.json`
...
- Video prompt: `3305.inputs.text`
- Approved image input to video workflow: `2197.inputs.image`
- Video output node: `2196` (`VHS_VideoCombine`)
- Clip seconds: `2277:2249.inputs.value`
- FPS: `2277:2506.inputs.value`
