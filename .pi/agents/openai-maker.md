---
name: openai-maker
description: Generates still-image attempts through the ChatGPT/OpenAI web UI using Playwright MCP and the user's subscription
...
4. Confirm login by finding the profile/sidebar and the textbox named `Chat with ChatGPT`. If login/account checks appear, stop for user action.
...
6. Fill the textbox named `Chat with ChatGPT` with the clean prompt and press Enter. In tool form, use `browser_type` targeting the textbox with `submit: true`.
...
Playwright extraction guidance:
- Prefer the network-response method because it preserves the full generated image and avoids fragile download-button UI.
- After the image appears, call `browser_network_requests` with:
  ```json
  {"static": true, "filter": "estuary/content"}
  ```
- Pick the newest/highest-numbered successful `[GET] .../backend-api/estuary/content?... => [200]` request for the just-generated image.
- Save its response body with `browser_network_request`, using a workspace-relative filename:
  ```json
  {
    "index": <REQUEST_INDEX>,
    "part": "response-body",
    "filename": "work/<song>/tmp/openai-maker/scene-0001-attempt-1-network-body.bin"
  }
  ```
- Convert that raw body to the final attempt PNG with FFmpeg and register it. The tested path saved a PNG body such as `1672x941`, then converted cleanly to `1280x720`.
- If the network request is missing, use `browser_run_code_unsafe` only to list the latest generated image `src`/dimensions from `document.images`; then navigate to that image URL in the browser to create a fresh network request, save the response body as above, and navigate back to the stored chat URL.
...
## Notes
Any ChatGPT/OpenAI login, quota, refusal, UI, download, or Playwright MCP issue, if relevant.
