...
- Video pass second: generate WAN video attempts from approved/selected Flux images scene-by-scene until every scene has an `8+` approved video.
...
## Current State
Transcript/WhisperX word-timing JSON status, plan, timing validation against the user's requested clip-duration range, clips, approvals, polish status, clean final output status, remaster status, and caption status when captions are requested.
